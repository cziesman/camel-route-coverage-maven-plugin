 ...
 */
package com.redhat.camel.route.coverage.map;
    ...
    private static class UnmodifiableEntrySet<K, V> implements Set<Map.Entry<K, List<V>>>, Serializable {
        ...
        private void filterArray(Object[] result) {
            ...
            for (int i = 0; i < result.length; i++) {
                if (result[i] instanceof Map.Entry<?, ?>) {
                    result[i] = new UnmodifiableEntry<>((Entry<K, List<V>>) result[i]);
                }
            }
        }
        ...
        public boolean equals(Object obj) {
            ...
            if (this == obj) {
                ...
                return ((Set<?>) obj).size() == this.delegate.size() &&
                        containsAll((Set<?>) obj);
            }
            return false;
        }
        ...
        private void filterArray(Object[] array) {
            ...
            for (int i = 0; i < array.length; i++) {
                if (array[i] instanceof List<?>) {
                    array[i] = Collections.unmodifiableList((List<?>) array[i]);
                }
            }
        }
